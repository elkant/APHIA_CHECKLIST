 ...
 */
public class save_editedMOH extends HttpServlet {
HttpSession session;
...
try{
       if(columnName.equalsIgnoreCase("First Name")){
          String update="UPDATE moh_staff SET fname=? WHERE id=?"; 
         ...
         conn.prest.executeUpdate();  
          ...
          String update="UPDATE moh_staff SET mname=? WHERE id=?"; 
         conn.prest=conn.conn.prepareStatement(update);
         conn.prest.setString(1, value);
         conn.prest.setString(2, id);  
         conn.prest.executeUpdate();  
          ...
          String update="UPDATE moh_staff SET lname=? WHERE id=?"; 
         conn.prest=conn.conn.prepareStatement(update);
         conn.prest.setString(1, value);
         conn.prest.setString(2, id);  
         conn.prest.executeUpdate();  
       ...
       {
          String update="UPDATE moh_staff SET phone=? WHERE id=?"; 
         conn.prest=conn.conn.prepareStatement(update);
         conn.prest.setString(1, value);
         conn.prest.setString(2, id);  
         conn.prest.executeUpdate();
  ...
  System.out.println("SQL statement is not executed!"+s);
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(save_editedMOH.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
